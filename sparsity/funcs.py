...
def std_redistribution(masking, name, weight, mask):
    """
    std_redistribution will redistribute the overall growth to layers with large SDR uncertainty
    """
    # TODO: rewrite the sds list into a dict such that we can directly access the std by name of the layer!
    sds_weight = None
    for name_, p in masking.sds:
        if name_ == name:
            sds_weight = p
    mean_stds = sds_weight[mask.bool()].mean().item()
    return mean_stds
...
def stable_std_prune(masking, mask, weight, name):
    # f_decay(t) * 1 - s^l the amount of pruned weights each layer
    # f_decay(t) == name2prune_rate[name] and
    # 1 - s^l == masking.init_nonzeros[name]
    num_remove = math.ceil(masking.name2prune_rate[name]*masking.init_nonzeros[name])

    # we need the sds of the weights in the input!
    sds_weights=[]
    for name_, p in masking.sds:
        if name_ == name:
            sds_weights.append(p)

    sds_weights = torch.abs(sds_weights[0].data)*mask # set all stds for non active weights to zero
    # choose num_remove weights with the highest stds out of the active weights in the layer
    x, idx = torch.sort(sds_weights.data.view(-1), descending=True)
    mask.data.view(-1)[idx[:num_remove]] = 0.0

    return mask

def std_prune(masking, mask, weight, name):
    # f_decay(t) * 1 - s^l the amount of pruned weights each layer
    # f_decay(t) == name2prune_rate[name] and
    # 1 - s^l == masking.init_nonzeros[name]
...
def stable_magnitude_prune(masking, mask, weight, name):
    ### num_remove == number of weights to be removed
    num_remove = math.ceil(masking.name2prune_rate[name]*masking.init_nonzeros[name])
    # get number of zeros that we need to index through to get to the lowest nonzero magnitude weights
    num_zeros = masking.name2zeros[name]
    ### we do k = num_zeros + num_remove because later we must index through all the zero weights
    ### num_remove are the additional weights we want to prune at the indiv. pruning step
    k = math.ceil(num_zeros + num_remove)
    if num_remove == 0.0: return weight.data != 0.0 # return the binary mask that hasn't changed

    x, idx = torch.sort(torch.abs(weight.data.view(-1)))
    ### here: we are indexing through all the zero weights too so we must do k = num_zeros + num_remove
...
def random_growth(masking, name, new_mask, total_regrowth, weight, device):
    n = (new_mask==0).sum().item()
    if n == 0: return new_mask
    expeced_growth_probability = (total_regrowth/n)
    # calling .cuda() wont work on CPU devices!!! only on GPU !
    new_weights = torch.rand(new_mask.shape).cuda() < expeced_growth_probability
    return new_mask.bool() | new_weights

# implement gradient based growth function!
def gradient_growth(masking, name, new_mask, total_regrowth, weight):
    # masking.name2prune_rate[name] == f_decay(t)
    # masking.init_nonzeros[name] == (1 - s^l)*N^l for the layer l==name
    regrowth = math.ceil(masking.name2prune_rate[name]*masking.init_nonzeros[name])
    zero_weight_mask = torch.logical_not(new_mask) # new_mask == mask given after the drop process

    grad = weight.grad.data*zero_weight_mask # all gradients belonging to active weights are set to zero

    x, idx = torch.sort(torch.abs(grad).flatten(), descending=True)
    new_mask.data.view(-1)[idx[:regrowth]] = 1.0

    return new_mask

def redistributed_gradient_growth(masking, name, new_mask, total_regrowth, weight):
    zero_weight_mask = torch.logical_not(new_mask) # new_mask == mask given after the drop process
...
prune_funcs = {}
prune_funcs['std_prune'] = std_prune
prune_funcs['stable_std_prune'] = stable_std_prune
...
prune_funcs['stable_magnitude'] = stable_magnitude_prune

growth_funcs = {}
growth_funcs['redistributed_gradient'] = redistributed_gradient_growth
...
redistribution_funcs['reverse_std_redistribution'] = std_redistribution
redistribution_funcs['std_redistribution'] = std_redistribution
