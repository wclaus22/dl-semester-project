"""
Code taken from https://github.com/TimDettmers/sparse_learning following
Dettmers T., Zettlemoyer L. 2019: Sparse Networks from Scratch: Faster Training without Losing Performance
https://arxiv.org/pdf/1907.04840.pdf

any triple comments like ### are by wclaus for explaining and understanding the code
any single comments like # are by the original authors
"""
...
class Masking(object):
    ...
    tb_writer=None):
        ...
        growth_modes = ['gradient', 'random', 'momentum', 'momentum_neuron']
        if growth_mode not in growth_modes:
            print('Growth mode: {0} not supported!'.format(growth_mode))
            print('Supported modes are:', str(growth_modes))

        ### need to define CUDA device, such that it will be compatible with both cuda and cpu!
        self._device = device
        ### need to define class-wide list of standard deviations of the weights that is updated every time the SDR algorithm does that!
        self.sds = sds_init
        self.zeta = zeta_init
        ### need a self.writer variable to be None at default, will be filled with the real writer if tb_logdir is not None
        ...
        self.name2nonzeros = {}
        self.name2removed = {}
        # i need this for stable prune rates
        self.init_nonzeros = {}
        ...
        ### RigL updates the mask after deltaT iterations:
        self.prune_every_k_steps = deltaT
        self.half = fp16
        self.name_to_32bit = {}

    # we need a function that will update the standard deviations from the SDR algorithm
    # and feed them into the Masking class!
    def get_sds(self, sds, zeta):
        self.sds = sds
        self.zeta = zeta
    ...
    def init(self, mode='constant', density=0.05):
        # this will initialize the first mask randomly for the given sparsity!
        # we could add additional kernel distributions ie. erdos renyi etc. here ?
        ...
        if mode == 'constant':
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    # self.masks is a dict with {weight_name: weight_tensor}
                    # at time of init self.masks is an empty dict
                    self.masks[name][:] = (torch.rand(weight.shape) < density).float().data.to(self._device, dtype=torch.float) ### i guess this is a tensor with all zeros and ones
                    self.baseline_nonzero += weight.numel()*density
                    self.init_nonzeros[name] = (self.masks[name] == 1.0).sum().item()
            self.apply_mask()

            # tb log initial non zero masked values or sparsity ratios for each layer
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    if name in self.name_to_32bit:
                        print('W2')
                    self.masks[name][:] = (weight != 0.0).float().data.to(self._device, dtype=torch.float)
                    self.baseline_nonzero += weight.numel()*density
            self.apply_mask()
        elif mode == 'linear':
            ...
            for name, weight in module.named_parameters():
                ...
                self.masks[name][:] = (torch.rand(weight.shape) < prob).float().data.to(self._device, dtype=torch.float)
            self.apply_mask()
    ...
    def step(self):
        ### the usual optimizer step for the ResNet
        self.optimizer.step()
        ### apply mask, doesn't prune anything yet! only makes sure weights are masked into sparse versions
        self.apply_mask()
        ### cosine annealing
        self.prune_rate_decay.step()
        ### get_dr will get the current alpha value in the cosine annealing
    ...
    def add_module(self, module, density, sparse_init='constant'):
        ...
        for name, tensor in module.named_parameters():
            ...
            self.masks[name] = torch.zeros_like(tensor, dtype=torch.float32, requires_grad=False).to(self._device, dtype=torch.float)
    ...
    def check_zeta(self):
        """
        zeta is an important parameter in SDR and will decay the standard deviations in the gaussian
        parameterization of the weight distributions

        as zeta becomes ever smaller, so do the stds and the amount of redistribution is normalized by the
        total sum of all mean standard deviations across all layers

        when this total mean becomes too small (empirically we found zeta ~ 1e-39),
        the redistribution function will become numerically unstable and
        redistribution will be halted, the training will revert to the default sparse method
        used, just without the std redistribution, sdr will keep on running but wont have any large
        influence anymore, as at zeta ~ 1e-39 the stds will have decayed very strongly
        """

        print('checking zeta and deciding to keep std redist. or revert back to no-redist. rigl/momentum mode')
        # when zeta becomes so small the std redistribution becomes useless and we revert back to
        print('zeta: ', self.zeta)
        print('redistribution: ', self.redistribution_mode)
        print('growth: ', self.growth_mode)
        print('prune: ', self.prune_mode)

        if self.zeta <= 1e-39 and (self.redistribution_mode=='std_redistribution' or self.redistribution_mode=='reverse_std_redistribution') and self.growth_mode=='redistributed_gradient':
            print('reverting back to RigL, deactivating std_redistribution to no redistribution')
            self.redistribution_func = redistribution_funcs['none']
            self.redistribution_mode = 'none'
            self.growth_func = growth_funcs['gradient']
            self.growth_mode = 'gradient'
            self.prune_func = prune_funcs['stable_magnitude']
            self.prune_mode = 'stable_magnitude'

        if self.zeta <= 1e-39 and (self.redistribution_mode=='std_redistribution' or self.redistribution_mode=='reverse_std_redistribution') and self.growth_mode=='momentum':
            print('reverting back to SNFS, deactivating std_redistribution to momentum redistribution')
            self.redistribution_func = redistribution_funcs['momentum']
            self.redistribution_mode = 'momentum'
            self.growth_func = growth_funcs['momentum']
            self.growth_mode = 'momentum'
            self.prune_func = prune_funcs['magnitude']
            self.prune_mode = 'magnitude'
    ...
    def gather_statistics(self):
        ...
        for name in self.name2variance:
            if self.total_variance != 0.0:
                ...
                if self.redistribution_mode =='reverse_std_redistribution':
                    # print('reverse std redist. mode! gotta reverse everything now!')
                    # will regrow weights where the stds are smallest!
                    self.name2variance[name] = (1 - self.name2variance[name])*(1./(len(list(self.name2variance.keys()))-1))
            else:
                print('Total variance was zero!')
                print('Training didnt revert back properly! check the check_zeta function!')
                print(self.growth_func)
                print(self.prune_func)
                print(self.redistribution_func)
                print(self.name2variance)

                # if the total variance (ie. normalized sum of average stds in layers) becomes too small,
                # stop the std redistribution and run no redistribution! (or any redist. for that matter)
                # if self.redistribution_mode == 'std_redistribution' or self.redistribution_mode == 'reverse_std_redistribution':
                #     print('going from std redist. to const uniform redist. factor, ie. none!')
                #     #self.redistribution_func = redistribution_funcs['none']
                #     #self.name2variance[name] = self.redistribution_func(self, name, weight, mask)
                #     self.name2variance[name] = 1.0/len(list(self.name2variance.keys()))
                # else:
                #     print('Total variance was zero!')
                #     print(self.growth_func)
                #     print(self.prune_func)
    ...
    def print_nonzero_counts_for_named_layer(self, name):
        mask = self.masks[name]
        num_nonzeros = (mask != 0).sum().item()
        sparsity_ratio = 1 - num_nonzeros/mask.numel() # num_nonzeros/number of elements in mask = density
        # if you want to log num_nonzeros you can do that too...
        return num_nonzeros, sparsity_ratio, mask.numel()
    ...
    def write_to_tb(self):
        ...
        for module in self.modules:
            for name, tensor in module.named_parameters():
                if name not in self.masks: continue
                num_nonzeros, sparsity_ratio, numels = self.print_nonzero_counts_for_named_layer(name)
                # only log linear weights and first layer conv weights from the 4 resnet blocks
                tot_nonzeros += num_nonzeros
                tot_els += numels
                if 'linear' in name:
                    self.writer.add_scalar(name + '_sparsity-ratio', sparsity_ratio, global_step=self.steps)
                    ...
                    self.writer.add_scalar(name + '_sparsity-ratio', sparsity_ratio, global_step=self.steps)
            self.writer.add_scalar('total network sparsity', 1 - tot_nonzeros/tot_els, global_step=self.steps)
